...
- **Site Search Palette** (`SiteSearch`, ⌘K/Ctrl+K): full-site quick finder over services, packages, FAQs and pages. Abyss panel, zero radius, four gold corner ticks, gold left-bar + gold tint on the selected row, uppercase micro group headings in gold, a stats footer (`12 SERVICES · 5 PACKAGES · 6 ANSWERS`) and an ↑↓/↵ hint bar; gold text caret. Trigger chip in the header (icon-only on mobile, `SEARCH ⌘K` chip on desktop) with gold hover. FAQ results deep-link to `#/faq?q=<id>` — the target question auto-expands with a 4s gold ring highlight (inset `ring-gold/40` + `bg-gold/[0.06]` fade) and scrolls to centre.
- **Count-Up Stat Numerals (admin)**: Overview stat cards animate their numeral from 0 → value over 750ms (ease-out cubic, rAF; `aria-label` carries the true value; jumps instantly under `prefers-reduced-motion`), giving the dashboard a live "arriving numbers" feel.
- **Gold Services Ticker** (`ServiceTicker`, home): a full-width `border-y border-[#202020]` strip on the abyss between the hero and the trust band — uppercase 11px `tracking-[0.24em]` steel test/package names separated by small gold hex glyphs, drifting in a seamless CSS marquee (duplicated track, `translateX(0 → -50%)`, 38s linear, `animation-play-state: paused` on hover; frozen to a static first frame under `prefers-reduced-motion` by the global rule). Abyss gradient fades on both edges so names materialise out of the dark. Screen-reader model: moving track is `aria-hidden`; a visually-hidden heading + list carries the names once.
- **Demand Ranking Bars (admin)**: "Most requested tests" card on the Overview — numbered rows (`01` in display font, gold/70 → gold on row hover), gold horizontal bars (`h-1.5`, leader `bg-gold`, rest `bg-gold/45` filling to group-hover gold) scaled to the top count, count in uppercase micro type right-aligned. Empty state invites the first booking.
- **Visit-List Chip Builder (booking)**: the Test or Package picker builds a bundle instead of a single choice — each pick appends a zero-radius chip (`border-white/15 bg-white/[0.05]`, gold `Check` tick, `×` remove button that inverts to `bg-gold text-black` on hover) with an `animate-in fade-in slide-in-from-bottom-1` entrance; a gold mono counter (`N SELECTED`, `aria-live`) rides the field label; the trigger placeholder flips to "Add another test…" once at least one chip exists. Duplicates and the 8-item cap surface a gold `role="status"` hint that self-dismisses after 2.8s. Bundles of 2+ add the "BUNDLED INTO ONE REQUEST" micro note and a gold `N TESTS` plate chip; the Aero-Cut summary plate stretches the test row full-width and joins labels with gold `+` separators. Preparation notes render one gold-bordered block per bundled test (max 3 visible, "+N more" line beyond).
- **Patient Confirmation Printout**: the booking success card carries a gold-outline "Print Confirmation" button that opens the shared print window (popup → hidden-iframe fallback, `src/lib/print-window.ts`) with a black-on-white document in the same paper language as the front-desk slip: brand head + tracking reference, request table, the on-screen QR tile lifted from the confirmation card DOM, patient/front-desk signature rules and a track-URL footnote.
